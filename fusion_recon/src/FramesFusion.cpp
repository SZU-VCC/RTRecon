...
bool FramesFusion::ReadLaunchParams(ros::NodeHandle & nodeHandle) {
	...
	//input odom topic
	nodeHandle.param("cloud_out_topic", m_sOutCloudTopic, std::string("/fusion_recon/processed_clouds"));

	//input point cloud topic
	nodeHandle.param("outcloud_tf_id", m_sOutCloudTFId, std::string("map"));

	//input odom topic
	nodeHandle.param("polygon_out_topic", m_sOutMeshTopic, std::string("/fusion_recon/surrounding_meshes"));
...
void FramesFusion::PublishMeshs(const pcl::PolygonMesh & oMeshModel){
	...
	InitMeshMsg(oMeshMsgs, 		m_sOutMeshTFId, 0, 0.9, 0.9, 0.9);
	InitMeshMsg(oMeshDynamic, 	m_sOutMeshTFId, 1, 0.9, 0.9, 0.9);
	...
	// add
	InitMeshMsg(oMeshAdded, 	m_sOutMeshTFId, 2, 0.9, 0.9, 0.9);

	// fused
	InitMeshMsg(oMeshFused,		m_sOutMeshTFId, 3, 0.9, 0.9, 0.9);
...
void FramesFusion::SurroundModeling(const pcl::PointXYZ & oBasedP, pcl::PolygonMesh & oCBModel, const int iFrameId){
	...
	PublishPointCloud(*pProcessedCloud, temp_feature, "/hush_fusion/temp_near_cloud");
...
void FramesFusion::SlideModeling(pcl::PolygonMesh & oResultMesh, const Eigen::Vector3f& vCenter, const int iFrameId) {
	...
	if(m_bUseUnionSetConnection) {
		...
		std::string sTopicName = "/fusion_recon/union_set";
		if(m_vDebugPublishers.count(sTopicName) == 0)
		{
	...
	std::string sTopicName = "/fusion_recon/static_expand";
	if(m_bDynamicDebug) {
...
void FramesFusion::SurfelFusionCore(pcl::PointNormal oLidarPos, pcl::PointCloud<pcl::PointNormal>& vDepthMeasurementCloud, pcl::PointCloud<pcl::PointNormal>& vPointCloudBuffer) {
	...
	// output dynamic
	PublishPointCloud(vPointCloudBuffer, associated_feature, "/fusion_recon/debug_associated_point");
}
