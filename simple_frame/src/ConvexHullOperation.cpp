...
========================================*/
void ConvexHullOperation::ChangeNormalType(const pcl::PointCloud<pcl::PointXYZI> &vClouds,
                                           const std::vector<FacePara> &vFaceParams,
                                           pcl::PointCloud<pcl::PointNormal> &vPNormals) {
...
Due to l = a*x_ref + b*y_ref + c*z_ref + d (n=(a,b,c),p=(x_ref,y_ref,z_ref)), we have l = n(*)p+d ,where (*) is dot
product matrix each variable L = N*P+D
========================================*/
Eigen::VectorXf ConvexHullOperation::PointToFaceDis(const Eigen::Vector3f &vVecP, const Eigen::MatrixXf &oMatN,
                                                    const Eigen::VectorXf &vfD) {
...
========================================*/
pcl::Normal ConvexHullOperation::CaculateTriangleNormal(pcl::PointXYZI &oPZero, pcl::PointXYZI &oPOne,
                                                        pcl::PointXYZI &oPTwo) {
    ...
    if (fLen == 0.0) {
        ...
        LOG(WARNING) << "something wrong on normal vector calculation!";
    } else {
...
========================================*/
void ConvexHullOperation::CaculateTriangleNormal(const pcl::PointXYZI &oPZero, const pcl::PointXYZI &oPOne,
                                                 const pcl::PointXYZI &oPTwo, const pcl::PointXYZI &oInnerP,
                                                 FacePara &oFacePara) {
...
void ConvexHullOperation::CaculateTriangleNormal(const pcl::PointCloud<pcl::PointXYZI> &vVertices,
    ...
    // Normalization
    if (abs(fLen - 0.0) == 0.0) {
		// TODO: 三角面片三个点近似一条直线，norm length为0 目前不处理
        // throw Exception();
        //  std::cout << "something wrong on normal vector calculation!" << std::endl;
        // LOG(WARNING) << "something wrong on normal vector calculation! ";
    } else {
...
Function: triangular plane equation calculation with a base point (reference)
========================================*/
void ConvexHullOperation::CaculateTriangleNormal(const Eigen::Vector3f &oRefPoint, 
												 const pcl::PointNormal &oPNormal,
...
========================================*/
void ConvexHullOperation::CaculateTriangleNormal(const pcl::PointXYZI &oPZero, const pcl::PointXYZI &oPOne,
                                                 const pcl::PointXYZI &oPTwo, FacePara &oFacePara) {
...
========================================*/
void ConvexHullOperation::ComputeCenterPoint(const pcl::PointCloud<pcl::PointXYZI> &vVertices,
...
Function: compute the parameters of all plane equations and save them in matrix form, which is convenient for subsequent
calculations
========================================*/
void ConvexHullOperation::ComputeAllFaceParams(const pcl::PointCloud<pcl::PointXYZI> &vVertices,
                                               const std::vector<pcl::Vertices> &vMeshVertexIdxs,
                                               pcl::PointXYZI &oCenterPoint, std::vector<FacePara> &vFaces) {
...
void ConvexHullOperation::ComputeAllFaceParams(const pcl::PointCloud<pcl::PointXYZI> &vVertices,
                                               const std::vector<pcl::Vertices> &vMeshVertexIdxs,
                                               pcl::PointXYZI &oCenterPoint, Eigen::MatrixXf &oMatNormal,
...
void ConvexHullOperation::ComputeAllFaceParams(const pcl::PointXYZI &oViewPoint,
                                               ...
                                               std::vector<pcl::Vertices> &vMeshVertexIdxs, Eigen::MatrixXf &oMatNormal,
                                               Eigen::VectorXf &vfDParam) {
...
void ConvexHullOperation::ComputeAllFaceParams(const pcl::PointXYZI &oViewPoint,
                                               ...
                                               pcl::PointCloud<pcl::PointXYZI> &vClouds,
...
void ConvexHullOperation::ComputeAllFaceParams(const pcl::PointCloud<pcl::PointXYZI> &vVertices,
    ...
    for (int i = 0; i != vMeshVertexIdxs.size(); ++i) {
        ...
        CaculateTriangleNormal(vVertices.points[vMeshVertexIdxs[i].vertices[0]],
                               vVertices.points[vMeshVertexIdxs[i].vertices[1]],
                               vVertices.points[vMeshVertexIdxs[i].vertices[2]], oOneFaceParam);
...
========================================*/
void ConvexHullOperation::NDToFacePara(const Eigen::MatrixXf &oMatN, const Eigen::VectorXf &vDParam,
...
the function computes the signed distance from each query point to the reconstructed surface
========================================*/
std::vector<float> ConvexHullOperation::ComputePointSignedDis(const pcl::PointCloud<pcl::PointXYZI> &vQueryCloud,
                                                              const Eigen::MatrixXf &m_oMatN,
                                                              const Eigen::VectorXf &m_vD) {
    ...
    // return null
    if (!bParamFlag) {
        std::cout << "WARNING! For convex hull operations, the normal vector of face has not been computed!"
                  << std::endl;
        return vAllPointSignedDis;
    }
...
std::vector<float> ConvexHullOperation::ComputePointSignedDis(const std::vector<SignedDis> &vConvexSD,
    ...
    for (int i = 0; i != vQueryCloud.points.size(); ++i) {
        ...
        float fOneSignedDis = (vQueryCloud.points[i].x - vCenterPoints[iFaceNum].x) *
                                  (vQueryCloud.points[i].x - vCenterPoints[iFaceNum].x) +
                              (vQueryCloud.points[i].y - vCenterPoints[iFaceNum].y) *
                                  (vQueryCloud.points[i].y - vCenterPoints[iFaceNum].y) +
                              (vQueryCloud.points[i].z - vCenterPoints[iFaceNum].z) *
                                  (vQueryCloud.points[i].z - vCenterPoints[iFaceNum].z);
