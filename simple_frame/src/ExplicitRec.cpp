		...
		thread_func[i] = [&, i, bMultiThread]() { 
			...
			if (vPointSecIdxs[i].size() > m_iSectorMinPNum){
				...
				if (m_bElevationFlag){
					...
					oViewGroundP.x = m_oViewPoint.x;
					oViewGroundP.y = m_oViewPoint.y;
					oViewGroundP.intensity = -1; // helper point flag
					oViewGroundP.z = m_oViewPoint.z - m_fViewElevation;
					pSectorCloud->push_back(oViewGroundP);
					oViewGroundP.z = m_oViewPoint.z + m_fViewElevation * 2;
					pSectorCloud->push_back(oViewGroundP);
				}
...
void ExplicitRec::OriginalAndShowReconstruction(const pcl::PointCloud<pcl::PointXYZI> & vSceneCloud, pcl::PointCloud<pcl::PointNormal> & vScenePNormal, const int line_min, const int line_max) {
	...
	m_vAllSectorClouds.clear();
	m_vAllSectorClouds.resize(vPointSecIdxs.size());
	m_vAllShowClouds.clear();
	m_vAllShowClouds.resize(vPointSecIdxs.size());

	//triangular face in each sector
	m_vAllSectorFaces.clear();
	m_vAllSectorFaces.resize(vPointSecIdxs.size());
	m_vAllShowFaces.clear();
	m_vAllShowFaces.resize(vPointSecIdxs.size());
	...
	for(int i = 0; i < vPointSecIdxs.size(); ++i) {
		...
		thread_func[i] = [&, i, bMultiThread]() { 
			...
			if (vPointSecIdxs[i].size() > m_iSectorMinPNum){
				...
				//可视化mesh
				// std::vector<pcl::Vertices> vOneShowFaces;
				// for (int j = 0; j != vTrueFaceStatus.size(); ++j){

				// 	if ( vTrueFaceStatus[j] || false ){			// TODO: 是否要剔除面片呢?
				// 		pcl::Vertices oOneFace(vOneFaces[j]);
				// 		vOneShowFaces.push_back(oOneFace);
				// 	}//end if

				// }//end for j != vTrueFaceStatus.size()

				//collect the vertices and faces in each sector
				m_vAllShowClouds[i] = pSectorCloud;
				// m_vAllShowFaces[i] = vOneShowFaces;
				

				//collect the vertices and faces in each sector
				pSectorCloud->push_back(m_oViewPoint);

				int minID = 1000, maxID = 0;//剔除部分面 相关参数 
				//for each face
				for (int j = 0; j < vOneFaces.size(); ++j){
					...
					int bAllZeroLine = 0, bAllMaxLine = 0;//剔除部分面 相关参数 
					...
					for (int k = 0; k < vOneFaces[j].vertices.size(); ++k){
						...
						// 面剔除
						if(pSectorCloud->at(iVertexSectorIdx).intensity <= line_min) bAllZeroLine |= 1 << j;
						if(pSectorCloud->at(iVertexSectorIdx).intensity >= line_max) bAllMaxLine |= 1 << j;
						minID = std::min(int(pSectorCloud->at(iVertexSectorIdx).intensity), minID);
						maxID = std::max(int(pSectorCloud->at(iVertexSectorIdx).intensity), maxID);
					...
					if (!bAllZeroLine && !bAllMaxLine)
						if ( vTrueFaceStatus[j] || false )
							m_vAllShowFaces[i].push_back(vOneFaces[j]);
				}//end j
				...
				std::vector<pcl::Vertices> vOneNewFaces;
				m_vAllSectorFaces[i] = vOneNewFaces;
				m_vAllShowFaces[i] = vOneNewFaces;
...
void ExplicitRec::OutputAllShowMeshes(pcl::PointCloud<pcl::PointXYZI> & vCloud){

	//relaitionships of vertexs in all point clouds
	 vCloud.points.clear();

	//for each sector
	for (int i = 0; i != m_vAllShowFaces.size(); ++i){

		//for each face
		for (int j = 0; j != m_vAllShowFaces[i].size(); ++j){

			//for each face vertex id
			for (int k = 0; k != m_vAllShowFaces[i][j].vertices.size(); ++k){

				//vertex id in each sector
				int iVertexSectorIdx = m_vAllShowFaces[i][j].vertices[k];

				//vertex id in all data
				vCloud.points.push_back(m_vAllShowClouds[i]->points[iVertexSectorIdx]);
				vCloud.points.back().intensity = m_vFaceWeight[i][j];
