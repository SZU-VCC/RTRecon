...
void SimpleRecon::LazyLoading() {
	...
	odomSuber = nodeHandle.subscribe(subOdomTopic, 2, &SimpleRecon::HandleTrajectory, this);	//记录运动信息到 m_vOdomHistory 的循环数组之中，并且 m_iTrajCount++

	//subscribe (hear) the point cloud topic 
	cloudSuber = nodeHandle.subscribe(subCloudTopic, 1, &SimpleRecon::HandlePointClouds, this);	//在m_vMapPCN记录法向点集，发布Mesh主题

	//***publisher related*** 
	//publish point cloud after processing
	FramePointsNormalPublisher = nodeHandle.advertise<sensor_msgs::PointCloud2>(pubFramePointsNormalTopic, 2, true);	//发布含有法向量的点云

  	//publish polygon constructed from one frame point cloud
	FrameMeshPublisher = nodeHandle.advertise<visualization_msgs::Marker>(pubFrameMeshTopic, 1);		//在接受到点云重建完之后， 被 PublishMeshs() 函数调用

    AdditionalPointsPublisher = nodeHandle.advertise<sensor_msgs::PointCloud2>(pubAdditionalPointsTopic, 1, true); //发布补充的点云
}
...
void SimpleRecon::HandlePointClouds(const sensor_msgs::PointCloud2 & vLaserData)
{
	...
	if (!(pcFrameCount % frameSmpNum)){ //根据帧采样频率记录
		...
		totalFrameNum = vLaserData.header.seq;
        ...
        // 发布点云/网格的可视化消息
        PublishPointCloud(vDisplayAdditionalPoints, AdditionalPointsPublisher);
        ...
        if(m_bCombineTwoClouds) {
            ...
            //一起发布消息
            PublishPointCloud(vCombinedCloud,FramePointsNormalPublisher);
        }
        else {
            ...
            //分别发布消息
            PublishPointCloud(vMeshCloudWithNormal,FramePointsNormalPublisher);
            vAdditionalPoints.is_dense = false;
            PublishPointCloud(vAdditionalPoints, AdditionalPointsPublisher);
        }
...
void SimpleRecon::SaveMesh(const pcl::PointCloud<T>& pointcloud, const std::vector<pcl::Vertices>& triangles) {
    ...
    std::stringstream ss;
    ss << sfOutFileHead << "single_img_clouds/";
    std::string command = "mkdir -p " + ss.str();
    system(command.c_str());
    ss << pcFrameCount << "_processed_mesh.ply";
