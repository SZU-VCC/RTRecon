...
public:
	...
	void OriginalAndShowReconstruction(const pcl::PointCloud<pcl::PointXYZI> & vSceneCloud, pcl::PointCloud<pcl::PointNormal> & vScenePNormal, const int line_min, const int line_max);
	...
	void OutputAllShowMeshes(pcl::PointCloud<pcl::PointXYZI> & vCloud);
	...
	//单帧重建的可视化
	std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> m_vAllShowClouds;
	std::vector<std::vector<pcl::Vertices>> m_vAllShowFaces;
